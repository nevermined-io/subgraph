...
describe('AccessTemplate', () => {
  ...
  describe('AgreementCreated', () => {
    ...
    it('should create a new agreement', async () => {
      ...
      assert.isTrue(receipt.events!.some(e => e.event === 'AgreementCreated'))
    ...
    it('should subscribe and wait for the event', async () => {
      ...
      assert.deepEqual(eventData._timeLocks, expectedEventData._timeLocks.map(v => v.toString()))
      assert.deepEqual(eventData._timeOuts, expectedEventData._timeOuts.map(v => v.toString()))
    })
