...
export class NeverminedConfigChange extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save NeverminedConfigChange entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("NeverminedConfigChange", id.toString(), this);
    }
  }

  static load(id: string): NeverminedConfigChange | null {
    return changetype<NeverminedConfigChange | null>(
      store.get("NeverminedConfigChange", id)
    );
  }
