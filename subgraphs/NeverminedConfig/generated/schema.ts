...
export class NeverminedConfigChange extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    ...
    assert(
      id != null,
      "Cannot save NeverminedConfigChange entity without an ID"
    );
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type NeverminedConfigChange must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("NeverminedConfigChange", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): NeverminedConfigChange | null {
    return changetype<NeverminedConfigChange | null>(
      store.get_in_block("NeverminedConfigChange", id.toHexString())
    );
  }

  static load(id: Bytes): NeverminedConfigChange | null {
    return changetype<NeverminedConfigChange | null>(
      store.get("NeverminedConfigChange", id.toHexString())
    );
  }
