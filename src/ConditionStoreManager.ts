...
const MAX_PAGE = 1000;
export type ConditionCreatedFilter = {
    id?: string | null;
    id_not?: string | null;
    id_gt?: string | null;
    id_lt?: string | null;
...
export type ConditionCreatedResult = {
    id: string;
    _typeRef: string;
    _who: string;
};
export type ConditionCreatedFields = {
    id: true;
    _typeRef: true;
    _who: true;
};
export type ConditionCreatedArgs<K extends keyof ConditionCreatedResult> = {
    [Property in keyof Pick<ConditionCreatedFields, K>]: ConditionCreatedFields[Property];
};
export const getConditionCreatedById = async function <K extends keyof ConditionCreatedResult>(url: string, options: SingleQueryOptions, args: ConditionCreatedArgs<K>): Promise<Pick<ConditionCreatedResult, K>> {
    const res = await axios.post(url, {
        query: generateGql("conditionCreated", options, args)
    });
    ...
    if (obj["_typeRef"])
        formattedObj["_typeRef"] = obj["_typeRef"];
    if (obj["_who"])
        formattedObj["_who"] = obj["_who"];
    return formattedObj as Pick<ConditionCreatedResult, K>;
};
export const getConditionCreateds = async function <K extends keyof ConditionCreatedResult>(url: string, options: MultiQueryOptions<ConditionCreatedFilter, ConditionCreatedResult>, args: ConditionCreatedArgs<K>): Promise<Pick<ConditionCreatedResult, K>[]> {
    const paginatedOptions: Partial<MultiQueryOptions<ConditionCreatedFilter, ConditionCreatedResult>> = { ...options };
    let paginationKey: keyof ConditionCreatedFilter | null = null;
    let paginationValue = "";
    if (options.first && options.first > MAX_PAGE) {
        ...
        paginationKey = paginatedOptions.orderBy + (paginatedOptions.orderDirection === "asc" ? "_gt" : "_lt") as keyof ConditionCreatedFilter;
        paginatedOptions.where = { ...options.where };
    }
    let results: Pick<ConditionCreatedResult, K>[] = [];
    do {
        if (paginationKey && paginationValue)
            paginatedOptions.where![paginationKey] = paginationValue as any;
        const res = await axios.post(url, {
            query: generateGql("conditionCreateds", paginatedOptions, args)
        });
        const r = res.data as any;
        if (r.errors && r.errors.length) {
            throw new Error(r.errors[0].message);
...
export type ConditionUpdatedFilter = {
    id?: string | null;
    id_not?: string | null;
    id_gt?: string | null;
    id_lt?: string | null;
...
export type ConditionUpdatedResult = {
    id: string;
    _typeRef: string;
    _state: number;
    _who: string;
};
export type ConditionUpdatedFields = {
    id: true;
    _typeRef: true;
    _state: true;
    _who: true;
};
export type ConditionUpdatedArgs<K extends keyof ConditionUpdatedResult> = {
    [Property in keyof Pick<ConditionUpdatedFields, K>]: ConditionUpdatedFields[Property];
};
export const getConditionUpdatedById = async function <K extends keyof ConditionUpdatedResult>(url: string, options: SingleQueryOptions, args: ConditionUpdatedArgs<K>): Promise<Pick<ConditionUpdatedResult, K>> {
    const res = await axios.post(url, {
        query: generateGql("conditionUpdated", options, args)
    });
    const r = res.data as any;
    if (r.errors && r.errors.length) {
        throw new Error(r.errors[0].message);
    ...
    if (obj["_typeRef"])
        formattedObj["_typeRef"] = obj["_typeRef"];
    if (obj["_state"])
        formattedObj["_state"] = obj["_state"];
    if (obj["_who"])
        formattedObj["_who"] = obj["_who"];
    return formattedObj as Pick<ConditionUpdatedResult, K>;
};
export const getConditionUpdateds = async function <K extends keyof ConditionUpdatedResult>(url: string, options: MultiQueryOptions<ConditionUpdatedFilter, ConditionUpdatedResult>, args: ConditionUpdatedArgs<K>): Promise<Pick<ConditionUpdatedResult, K>[]> {
    const paginatedOptions: Partial<MultiQueryOptions<ConditionUpdatedFilter, ConditionUpdatedResult>> = { ...options };
    let paginationKey: keyof ConditionUpdatedFilter | null = null;
    let paginationValue = "";
    if (options.first && options.first > MAX_PAGE) {
        paginatedOptions.first = MAX_PAGE;
        paginatedOptions.orderBy = options.orderBy || "id";
        paginatedOptions.orderDirection = options.orderDirection || "asc";
        paginationKey = paginatedOptions.orderBy + (paginatedOptions.orderDirection === "asc" ? "_gt" : "_lt") as keyof ConditionUpdatedFilter;
        paginatedOptions.where = { ...options.where };
    }
    let results: Pick<ConditionUpdatedResult, K>[] = [];
    do {
        if (paginationKey && paginationValue)
            paginatedOptions.where![paginationKey] = paginationValue as any;
        const res = await axios.post(url, {
            query: generateGql("conditionUpdateds", paginatedOptions, args)
        });
        const r = res.data as any;
        if (r.errors && r.errors.length) {
            throw new Error(r.errors[0].message);
