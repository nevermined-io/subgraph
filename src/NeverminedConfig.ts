...
export type NeverminedConfigChangeFilter = {
    id?: string | null;
    id_not?: string | null;
    id_gt?: string | null;
    id_lt?: string | null;
...
export type NeverminedConfigChangeResult = {
    id: string;
    _whoChanged: string;
    _parameter: string;
};
export type NeverminedConfigChangeFields = {
    id: true;
    _whoChanged: true;
    _parameter: true;
};
export type NeverminedConfigChangeArgs<K extends keyof NeverminedConfigChangeResult> = {
    [Property in keyof Pick<NeverminedConfigChangeFields, K>]: NeverminedConfigChangeFields[Property];
};
export const getNeverminedConfigChangeById = async function <K extends keyof NeverminedConfigChangeResult>(url: string, options: SingleQueryOptions, args: NeverminedConfigChangeArgs<K>): Promise<Pick<NeverminedConfigChangeResult, K>> {
    const res = await axios.post(url, {
        query: generateGql("neverminedConfigChange", options, args)
    });
    const r = res.data as any;
    if (r.errors && r.errors.length) {
        throw new Error(r.errors[0].message);
    ...
    if (obj["id"])
        formattedObj["id"] = obj["id"];
    if (obj["_whoChanged"])
        formattedObj["_whoChanged"] = obj["_whoChanged"];
    if (obj["_parameter"])
        formattedObj["_parameter"] = obj["_parameter"];
    return formattedObj as Pick<NeverminedConfigChangeResult, K>;
};
export const getNeverminedConfigChanges = async function <K extends keyof NeverminedConfigChangeResult>(url: string, options: MultiQueryOptions<NeverminedConfigChangeFilter, NeverminedConfigChangeResult>, args: NeverminedConfigChangeArgs<K>): Promise<Pick<NeverminedConfigChangeResult, K>[]> {
    const paginatedOptions: Partial<MultiQueryOptions<NeverminedConfigChangeFilter, NeverminedConfigChangeResult>> = { ...options };
    let paginationKey: keyof NeverminedConfigChangeFilter | null = null;
    let paginationValue = "";
    if (options.first && options.first > MAX_PAGE) {
        paginatedOptions.first = MAX_PAGE;
        paginatedOptions.orderBy = options.orderBy || "id";
        paginatedOptions.orderDirection = options.orderDirection || "asc";
        paginationKey = paginatedOptions.orderBy + (paginatedOptions.orderDirection === "asc" ? "_gt" : "_lt") as keyof NeverminedConfigChangeFilter;
        paginatedOptions.where = { ...options.where };
    }
    let results: Pick<NeverminedConfigChangeResult, K>[] = [];
    do {
        if (paginationKey && paginationValue)
            paginatedOptions.where![paginationKey] = paginationValue as never;
        const res = await axios.post(url, {
            query: generateGql("neverminedConfigChanges", paginatedOptions, args)
        });
        const r = res.data as any;
        if (r.errors && r.errors.length) {
            throw new Error(r.errors[0].message);
