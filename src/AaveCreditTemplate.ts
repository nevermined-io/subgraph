...
export type VaultCreatedResult = {
    id: string;
...
export type VaultCreatedFields = {
    id: true;
    _vaultAddress: true;
    _creator: true;
    _lender: true;
...
export type VaultCreatedArgs<K extends keyof VaultCreatedResult> = {
    [Property in keyof Pick<VaultCreatedFields, K>]: VaultCreatedFields[Property];
};
export const getVaultCreatedById = async function <K extends keyof VaultCreatedResult>(url: string, options: SingleQueryOptions, args: VaultCreatedArgs<K>): Promise<Pick<VaultCreatedResult, K>> {
    const res = await axios.post(url, {
        query: generateGql("vaultCreated", options, args)
    });
    const r = res.data as any;
    if (r.errors && r.errors.length) {
        throw new Error(r.errors[0].message);
    ...
    if (obj["id"])
        formattedObj["id"] = obj["id"];
    if (obj["_vaultAddress"])
        formattedObj["_vaultAddress"] = obj["_vaultAddress"];
    if (obj["_creator"])
        formattedObj["_creator"] = obj["_creator"];
    if (obj["_lender"])
        formattedObj["_lender"] = obj["_lender"];
    if (obj["_borrower"])
        formattedObj["_borrower"] = obj["_borrower"];
    return formattedObj as Pick<VaultCreatedResult, K>;
};
export const getVaultCreateds = async function <K extends keyof VaultCreatedResult>(url: string, options: MultiQueryOptions<VaultCreatedFilter, VaultCreatedResult>, args: VaultCreatedArgs<K>): Promise<Pick<VaultCreatedResult, K>[]> {
    const paginatedOptions: Partial<MultiQueryOptions<VaultCreatedFilter, VaultCreatedResult>> = { ...options };
    let paginationKey: keyof VaultCreatedFilter | null = null;
    let paginationValue = "";
    if (options.first && options.first > MAX_PAGE) {
        paginatedOptions.first = MAX_PAGE;
        paginatedOptions.orderBy = options.orderBy || "id";
        paginatedOptions.orderDirection = options.orderDirection || "asc";
        paginationKey = paginatedOptions.orderBy + (paginatedOptions.orderDirection === "asc" ? "_gt" : "_lt") as keyof VaultCreatedFilter;
        paginatedOptions.where = { ...options.where };
    }
    let results: Pick<VaultCreatedResult, K>[] = [];
    do {
        if (paginationKey && paginationValue)
            paginatedOptions.where![paginationKey] = paginationValue as any;
        const res = await axios.post(url, {
            query: generateGql("vaultCreateds", paginatedOptions, args)
        });
        const r = res.data as any;
        if (r.errors && r.errors.length) {
            throw new Error(r.errors[0].message);
